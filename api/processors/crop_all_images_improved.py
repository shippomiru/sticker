...
def process_all_images(input_dirs, output_dir, name_pattern="_transparent.png", target_ratio=0.5, safety_padding=0.15):
    """批量处理所有符合条件的透明PNG图片"""
    # 确保输出目录存在
    ...
    for input_dir in input_dirs:
        pattern = os.path.join(input_dir, f"*{name_pattern}")
        png_files = glob.glob(pattern)
        all_png_files.extend(png_files)
    
    print(f"找到 {len(all_png_files)} 个透明PNG图片")
    ...
    for png_file in all_png_files:
        # 获取文件名（不含路径和扩展名）
        basename = os.path.basename(png_file).replace(name_pattern, "")
        
        # 定义输出文件路径
        output_path = os.path.join(output_dir, f"{basename}_cropped_safe.png")
        
        # 裁剪图片
        print(f"\n正在处理图片: {basename}")
        if crop_to_center_main_subject(png_file, output_path, target_ratio, safety_padding):
            success_count += 1
    
    print(f"\n所有图片处理完成! 成功处理 {success_count}/{len(all_png_files)} 张图片")
    print(f"请查看 {output_dir} 目录下的处理结果")

def main():
    # 输入和输出目录
    input_dirs = ["results", "results-photo2", "results-photo3"]
    output_dir = "results-cropped-safe"
    
    # 批量处理所有图片，增加15%的安全边距
    process_all_images(input_dirs, output_dir, target_ratio=0.5, safety_padding=0.15)
