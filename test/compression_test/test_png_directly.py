...
PNG_DIR = os.path.join(BASE_DIR, "results_method1")  # 使用方法1生成的PNG
METHOD1_DIR = os.path.join(BASE_DIR, "png_test_method1")  # pngquant + oxipng
METHOD2_DIR = os.path.join(BASE_DIR, "png_test_method2")  # pngquant + oxipng + squoosh
...
def method1_compression(input_png, output_png):
    """方法1: pngquant + oxipng"""
    # 步骤1: pngquant压缩
    temp_pngquant = os.path.join(TEMP_DIR, os.path.basename(input_png).replace(".png", "_pngquant.png"))
    if not compress_with_pngquant(input_png, temp_pngquant):
        print(f"pngquant压缩失败: {input_png}")
        shutil.copy2(input_png, temp_pngquant)  # 失败则使用原始PNG
...
def method2_compression(input_png, output_png):
    """方法2: pngquant + oxipng + squoosh"""
    # 步骤1-2: 与方法1相同
    method1_output = os.path.join(TEMP_DIR, os.path.basename(input_png).replace(".png", "_method1.png"))
    method1_compression(input_png, method1_output)
...
def process_images():
    ...
    print(f"找到 {len(png_files)} 张PNG图片进行测试")
    
    # 处理每一张图片
    for idx, png_file in enumerate(png_files):
        filename = os.path.basename(png_file)
        print(f"\n处理图片 [{idx+1}/{len(png_files)}]: {filename}")
        
        # 准备输出文件路径
        method1_output = os.path.join(METHOD1_DIR, filename)
        method2_output = os.path.join(METHOD2_DIR, filename)
        ...
        method1_compression(png_file, method1_output)
        ...
        start_time = time.time()
        method2_compression(png_file, method2_output)
        method2_time = time.time() - start_time
        method2_size = os.path.getsize(method2_output)
        
        # 计算SSIM (结构相似度)
        method1_ssim = calculate_ssim(png_file, method1_output)
        method2_ssim = calculate_ssim(png_file, method2_output)
...
def summarize_results():
    ...
    # 保存详细结果到JSON文件
    results_file = os.path.join(BASE_DIR, "png_test_results.json")
